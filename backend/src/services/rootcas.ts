...
// When running without Key Vault we persist root CA key & cert to disk so
// they survive process restarts (previous implementation kept them only in memory).
// NOTE: This is strictly for local development convenience. Do NOT use this
// pattern for production secret storage.
const localKeysDir = path.join(process.cwd(), 'data', 'rootkeys');
function persistLocalSecret(name: string, value: string) {
  try {
    if (!useLocalSecrets) return;
    if (!fs.existsSync(localKeysDir)) fs.mkdirSync(localKeysDir, { recursive: true });
    // name pattern: rootca-<id>-key | rootca-<id>-cert
    const match = name.match(/^rootca-(.+)-(key|cert)$/);
    if (!match) return;
    const id = match[1];
    const kind = match[2];
...
function loadLocalSecret(name: string): string | undefined {
  try {
    if (!useLocalSecrets) return undefined;
    if (localSecrets.has(name)) return localSecrets.get(name);
    const match = name.match(/^rootca-(.+)-(key|cert)$/);
...
// Danger zone helper: wipe all locally persisted root CAs & their keys (local dev only)
// Not exposed via HTTP; used by scripts/reset.ts for a clean slate during development.
export function clearAllRootCAs(): void {
  if (tableName) {
    console.warn('clearAllRootCAs skipped: running with table storage (not supported).');
    return;
  }
  try {
    localRootCAs.splice(0, localRootCAs.length);
    // remove persistence file
    if (fs.existsSync(rootFile)) fs.rmSync(rootFile, { force: true });
    // remove any cached local secrets & key files
    if (fs.existsSync(localKeysDir)) {
      for (const f of fs.readdirSync(localKeysDir)) {
        try { fs.rmSync(path.join(localKeysDir, f), { force: true }); } catch { /* ignore */ }
      }
    }
    localSecrets.clear();
...
export async function listRootCAs(): Promise<RootCAMetadata[]> {
  ...
  try {
    ...
    for await (const ent of tc.listEntities({ queryOptions: { filter: "PartitionKey eq 'rootca'" } })) {
      out.push({ id: ent.rowKey as string, displayName: ent.displayName as string, createdAt: ent.createdAt as string, certPem: ent.certPem as string });
    }
